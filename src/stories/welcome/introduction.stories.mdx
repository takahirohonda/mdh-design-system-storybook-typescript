...
We have a design system first approach. Any FED component should be created in the design system first, and then imported as a moudule.
<br />
...
However, use interface if there is any case that is suitable.
```typescript
export type MyProps = {
  hidden: boolean,
  content: text
...
- Use string Enum to limit the prop options when it is applicable.
...
- Create index.tsx file and export all the components withing the same folder.
...
- We would like to minimise the use of 3rd party library (e.g. do not use Bootstrap, jQueary, lodash and so on).
- Use yarn.
...
We are using Storybook v7. Many changes happend in v6. 
Some of the old APIs have been depricated. 
As a rule of thum, we use the convention recommended for v6 and above.
...
.storybook # storybook configuration folder
  ...
  manager-head.html # Inject elements globally. This is where title and favicons are.
